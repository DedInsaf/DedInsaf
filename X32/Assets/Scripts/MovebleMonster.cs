...
public class MovebleMonster : Monster
{
    ...
    protected override void OnTriggerEnter2D(Collider2D collider) //Посмотреть, когда по монстру попала пуля
    {
    ...
    private void Move()
    {
        ...
        if (colliders.Length > 0 && colliders.All(x => !x.GetComponent<Player>())) direction.x *= -1.0F;
